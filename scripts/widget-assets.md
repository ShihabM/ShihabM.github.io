...
Six accessoryRectangular renders use the app's `AccessoryRectangularViewMovies`, `AccessoryRectangularViewShows`, and `YUAccessoryRectangularViewMovies` views with The Odyssey, Toy Story 5, The Bear, Severance, Supergirl, and The Pitt. Each receives a single metadata item so its selection stays deterministic. Exported using the same iPhone simulator and light appearance at 170×73 points, 3× scale. The native Lock Screen content is displayed on a white presentation surface with 16-point corners, 8-point horizontal padding and 4-point vertical padding; these surfaces are for the website collage, not a simulated Lock Screen wallpaper. Pairs occupy the height of one small Home Screen widget. Existing Home Screen images remain in the expanded nine-column collage.
